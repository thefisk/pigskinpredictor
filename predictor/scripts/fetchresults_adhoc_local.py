...
def run():
    ...
    soup = BeautifulSoup(
        open("./predictor/scripts/manual_html/2020_week2.html"), 'lxml')
    ...
    for game in soup.find_all('th', {'data-stat': 'week_num', 'csk': week}):
        ...
        if (game.find_next('td', {'data-stat': 'winner'})).strong == None:
            winningteam = (team_dict[game.find_next(
                'td', {'data-stat': 'winner'}).text])
            if (game.find_next('td', {'data-stat': 'game_location'}).text == "@"):
                winner_location = "Away"
            else:
                winner_location = "Home"
            losingteam = (team_dict[game.find_next(
                'td', {'data-stat': 'loser'}).text])
